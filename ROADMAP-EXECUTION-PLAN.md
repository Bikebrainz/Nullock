...
- Wave 2 (2026-08-09, db0c788/8987a83): continued the Repeater editor thread -- in-editor find gains a case-sensitivity toggle and a regex mode (JS syntax, zero-length-match safe, invalid pattern fails closed instead of throwing); closes #348 (partial -> present). Decoder's Input/Output blocks get a Text/Hex view toggle (reuses proxy.jsx's toHexDump); left honestly partial (#358, absent -> partial) because transcode.cpp's hex-decode/base64-decode already collapse invalid UTF-8 via QString::fromUtf8 before the JSON reaches the UI, so a real non-UTF8 byte is lost server-side before the new view can show it -- fixing that needs a backend wire-format change this cloud session cannot build/verify. #362 (docked Inspector Selection widget) and #359 (docked Inspector panel itself) remain untouched partials, tracked separately. No backend touched this wave; build-windows/build-linux CI unaffected by construction, confirmed green below.
