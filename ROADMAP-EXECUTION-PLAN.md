...
- Wave 2 (2026-08-08, cd03a95/140d355): Repeater message editors gain Raw/Headers/Body/Preview/Hex view tabs (reusing proxy.jsx's renderView/toHexDump), a sandboxed (`sandbox=""`) srcdoc-iframe Render view for the response, in-editor find with wrap-around next/prev + match count, and a selection readout (char count + first-byte dec/hex, non-printing chars labelled). Closed #341, #342 (absent -> present); register-accuracy fix #347 (Ctrl+Space was already shipped in 1074f35, parity.json just never flipped) -> present; #348 and #362 landed real but incomplete (no regex/case-sensitivity; readout lives in Repeater's own panes, not a docked Inspector panel) -> partial, honestly.
