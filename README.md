...
- [x] **Extensions API** — drop `.js` files in `%APPDATA%/Nullock/Nullock/extensions/` and they run in a shared `QJSEngine` at startup (and on `extensions.reload()`). Global `nullock` object exposes `log(msg)`, `onResponse(fn)`, and `onRequest(fn)`. Each handler receives a JS entry with method/host/port/path/headers (array of [name,value])/bodyText/etc. **Returning a modified entry actually mutates the wire bytes**: header injection, body rewrites, method/path overrides all take effect before the request is serialized upstream / before the response is sent to the client. Cross-thread safe via `BlockingQueuedConnection` so worker threads call into the main-thread engine synchronously. Read-only handlers (no return / undefined) leave the bytes untouched. Smoke-tested end-to-end: a JS handler pushing `X-Nullock-Smoke: ok` onto the response headers, plus a curl through the proxy, confirms the injected header appears in curl's output.
