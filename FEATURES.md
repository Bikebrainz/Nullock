...
- [~] HTTP/2 multiplexing — **Phases 1 + 2 shipped**. Phase 1: the upstream `H2Client` is a session-scoped, N-concurrent-stream client (`sendConcurrent`) over one nghttp2 session + pump loop, node-stable per-stream state (`std::map<…,unique_ptr>` → `stream_user_data`), local concurrency + per-stream body caps, pure `h2_client_logic` seam (48-assert test). **Phase 2**: `H2Client::send()` reuses ONE persistent nghttp2 session across calls, and the proxy h2 branch is now a keep-alive loop, so sequential browser requests on a tunnel share the upstream TCP+TLS+h2 connection (no per-request handshake). Host-bypass is gated on genuine connection-level failure only (`connectionFailed`), a truncated body is still served, and a session is torn down rather than reused after any unclean stream. Phase 3 (browser-side h2 termination — ALPN + a server-role nghttp2 session) remains for full end-to-end multiplexing.
