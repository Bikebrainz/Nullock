...
- **Slowloris on control server.** Header read enforces a hard 3s wall-clock deadline per connection (separate from the per-`waitForReadyRead` budget) so a client dribbling one byte every 4.9s can't pin the main thread forever. Body read enforces a 30s deadline on the same basis.
