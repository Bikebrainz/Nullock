...
| **Network attacker between us and a real origin** | Replaces an upstream server's bytes in transit (e.g. by spoofing DNS or MITMing a Cloudflare-fronted host). | Upstream TLS verifies the peer chain and hostname unless the operator explicitly configures an exception for that host. An exception intentionally removes that protection for the selected host. |
...
- **Active-tool scope at the send boundary.** Repeater, Intruder (each generated request), chains, scanners, crawler links, Sequencer and redirect hops check the actual host, port, protocol and path before connecting. HTTP checks include the wire path and decoded dot-segment-normalized path; query strings are not part of path scope. An `always` redirect setting does not override project scope. Raw TCP/TLS inspection requires a path-unrestricted endpoint grant; an unknown protocol cannot consume an HTTP-only grant. Invalid advanced-scope edits are rejected, and invalid saved rules produce a deny policy. Update/marketplace downloads, certificate-transparency/WHOIS lookups and configured application services use separate operational connections. The passive browser proxy retains its logging/interception scope behavior. Verified by `scripts/outbound_scope_regression.py`, `Tests/scope_logic` and `Tests/proxy_shutdown`.
...
- **Control request intake.** Event-driven reads keep incomplete connections off the main thread. Headers have a three-second deadline; bodies have a five-second idle timeout and a thirty-second absolute deadline. At most 32 connections and 128 MiB of pending declared body bytes are admitted. Host, token and CSRF checks run before accepting a body. Duplicate framing/security headers and Transfer-Encoding are rejected. Response flushing is asynchronous and bounded. `scripts/control_responsiveness_regression.py` covers partial requests, framing rejection, budget release, deadlines and concurrent Repeater activity.
...
### Shutdown safety
- **Worker drain before teardown.** Shutdown stops new work, releases held intercepts and joins tracked proxy connection threads. It drains the global QtConcurrent pool while processing queued events, then processes pending results before destroying the application wiring. A five-second timeout no longer permits workers to outlive their owners.
