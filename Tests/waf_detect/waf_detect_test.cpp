...
std::optional<Detection> find(const QList<Detection> &ds, const char *name) {
...
int main(int argc, char **argv) {
    ...
    chk("cf-ray header presence -> Cloudflare (cdn)",
        [](){ const auto d = find(detectFromHeaders(H({{"cf-ray", "8abc-DFW"}})), "Cloudflare");
              return d && d->kind == "cdn"; }());
    chk("Server: cloudflare value substring -> Cloudflare",
        find(detectFromHeaders(H({{"Server", "cloudflare"}})), "Cloudflare") != std::nullopt);
    chk("__cf_bm cookie -> Cloudflare",
        find(detectFromHeaders(H({{"Set-Cookie", "__cf_bm=xyz; Path=/; Secure"}})), "Cloudflare") != std::nullopt);
    chk("x-iinfo header -> Imperva Incapsula (waf)",
        [](){ const auto d = find(detectFromHeaders(H({{"x-iinfo", "1-2-3"}})), "Imperva Incapsula");
              return d && d->kind == "waf"; }());
    chk("BIGipServer cookie NAME -> F5 BIG-IP (lb)",
        [](){ const auto d = find(detectFromHeaders(H({{"Set-Cookie", "BIGipServerpool_web=12345.20480.0000; path=/"}})), "F5 BIG-IP");
              return d && d->kind == "lb"; }());
    chk("Server: AkamaiGHost -> Akamai",
        find(detectFromHeaders(H({{"Server", "AkamaiGHost"}})), "Akamai") != std::nullopt);
    chk("x-amz-cf-id -> AWS CloudFront",
        find(detectFromHeaders(H({{"x-amz-cf-id", "abc=="}})), "AWS CloudFront") != std::nullopt);
    ...
    chk("Server: CloudFront -> AWS CloudFront (value-needle sibling)",
        [](){ const auto d = find(detectFromHeaders(H({{"Server", "CloudFront"}})), "AWS CloudFront");
              return d && d->kind == "cdn"; }());

    // ---- vendor-unique additions (this tick) ----------------------------
    chk("AWSALB cookie -> AWS ELB/ALB (lb)",
        [](){ const auto d = find(detectFromHeaders(H({{"Set-Cookie", "AWSALB=base64data; Expires=...; Path=/"}})), "AWS ELB/ALB");
              return d && d->kind == "lb"; }());
    chk("AWSALBCORS cookie (substring) -> AWS ELB/ALB",
        find(detectFromHeaders(H({{"Set-Cookie", "AWSALBCORS=b64; SameSite=None"}})), "AWS ELB/ALB") != std::nullopt);
    chk("x-sigsci-requestid header -> Signal Sciences (waf)",
        [](){ const auto d = find(detectFromHeaders(H({{"x-sigsci-requestid", "0123abcd"}})), "Signal Sciences");
              return d && d->kind == "waf"; }());
    ...
    chk("cf-ray wins over Server (evidence shows the header, not Server)",
        [](){ const auto d = find(detectFromHeaders(H({{"cf-ray", "8abc"}, {"Server", "cloudflare"}})), "Cloudflare");
    ...
    chk("BIGipServer in a cookie VALUE does NOT fire F5",
        find(detectFromHeaders(H({{"Set-Cookie", "session=BIGipServer_lookalike; path=/"}})), "F5 BIG-IP") == std::nullopt);
    chk("datadome in a cookie VALUE does NOT fire DataDome",
        find(detectFromHeaders(H({{"Set-Cookie", "ref=via-datadome-blog; path=/"}})), "DataDome") == std::nullopt);
    chk("NSC_ as a cookie VALUE does NOT fire NetScaler",
        find(detectFromHeaders(H({{"Set-Cookie", "tracker=NSC_payload; path=/"}})), "Citrix NetScaler") == std::nullopt);
    chk("datadome as the cookie NAME DOES fire DataDome",
        find(detectFromHeaders(H({{"Set-Cookie", "datadome=abc; path=/"}})), "DataDome") != std::nullopt);

    // ---- case-insensitivity --------------------------------------------
    chk("CF-RAY (uppercase header name) -> Cloudflare",
        find(detectFromHeaders(H({{"CF-RAY", "8abc"}})), "Cloudflare") != std::nullopt);
