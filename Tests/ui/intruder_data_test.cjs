...
const snapshot = () => ({seq:7, instanceId:'server', bootInfo:{project,
  projectDir:'/projects/' + project, historyGeneration:project}, intruder:{host}});
...
(async () => {
  ...
  const first = NL.actions.intruderSet({host:'a'});
  const second = NL.actions.intruderSet({host:'ab'});
  assert.equal(NL.intruder.host, 'ab');
  ...
  finishPoll(stale);
  assert.equal(NL.intruder.host, 'ab', 'a late poll cannot erase newer typing');
  assert.equal(NL._seq, 0, 'a skipped draft forces another snapshot');
  host = 'ab'; tick(); finishPoll(polls.shift());
  assert.equal(NL.intruder.host, 'ab');

  const edit = NL.actions.intruderSet({host:'before-start'});
  const start = NL.actions.intruderStart();
  await flush(); assert.equal(requests[0].url, '/api/intruder/set');
  finishWrite(requests.shift()); await edit; await flush();
  assert.equal(requests[0].url, '/api/intruder/start');
  finishWrite(requests.shift()); await start;

  const inFlight = NL.actions.intruderSet({host:'outgoing'});
  const queued = NL.actions.intruderSet({host:'must-not-leak'});
  ...
  assert.equal(NL.intruder.host, 'independent');
  ...
  assert.equal(NL._intruderPendingWrites, 0);

  const failed = NL.actions.intruderSet({host:'unsaved'});
  ...
  assert.equal(NL.intruder.host, 'independent');
  assert.equal(NL._intruderPendingWrites, 0);
  console.log('PASS: Intruder typing survives stale polls; writes/start are ordered; project changes and failures recover safely');
