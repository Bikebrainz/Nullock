...
QByteArray ControlServer::apiResponse(const QString &method, const QString &path,
    ...
    // Named session-acquisition macros (#10/#11). POST { macros: [...] } replaces
    // the set; any request returns the current set. A macro = a recorded login
    // sequence (ChainRunner steps) + sessionHost + optional loggedOut* condition
    // that auto-re-runs it. Steps share the /api/chain/run shape.
    ...
    if (path == "/api/session-macros/run") {
        ...
        // Dispatch off the control thread -- runMacro does blocking login I/O.
        auto *sr = m_wiring.sessionRules;
        (void)QtConcurrent::run([sr, name]() { sr->runMacro(name); });
        return okJson({{ "ok", true }, { "dispatched", name }});
    }
    ...
    if (path == "/api/report/build") {
        ...
        // Report customisation: optional custom title, an issue-selection filter,
        // and an appendix -- the same filter contract as /api/report/json, parsed
        // from the POST body.
        const ControlLogic::ReportFilterCriteria rfc = ControlLogic::reportFilterFromJson(bodyJson);
        const QString customTitle = bodyJson.value("title").toString();
        const QString titleText = customTitle.isEmpty()
            ? (QStringLiteral("Nullock engagement report -- ") + proj) : customTitle;
        out += "# " + ControlLogic::mdTextSafe(titleText) + "\n\n";
        ...
        out += "## Findings\n\n";
        if (m_wiring.scanner) {
            const auto allFindings = m_wiring.scanner->findings(0);
            // Apply the report-customisation filter (same rule as /api/report/json).
            QList<Nullock::Core::Finding> findings;
            for (const auto &f : allFindings)
                if (ControlLogic::findingPasses(f.severity, f.confidence, f.kind, /*fixed=*/false,
                                                rfc.minSeverityRank, rfc.minConfidenceRank,
                                                rfc.includeKinds, rfc.excludeKinds, rfc.includeFixed))
                    findings.append(f);
            if (findings.size() != allFindings.size())
                out += "_(showing " + QString::number(findings.size()) + " of "
                     + QString::number(allFindings.size()) + " findings after filtering)_\n\n";
        ...
        const QString appendix = bodyJson.value("appendix").toString();
        if (!appendix.isEmpty()) {
            out += "## Appendix\n\n";
            out += ControlLogic::mdTextSafe(appendix) + "\n\n";
        }
    ...
    if (path == "/api/report/html") {
        ...
        const QString proj = m_wiring.projectStore
            ? m_wiring.projectStore->metadata().name : QStringLiteral("default");
        // Report customisation: optional custom title + issue-selection filter +
        // appendix from the POST body. A standalone HTML report reflects the
        // INCLUDED set throughout (grade/cards/table), matching Burp's report
        // wizard (the operator chose the filter), so filter BEFORE the rollup.
        const ControlLogic::ReportFilterCriteria rfc = ControlLogic::reportFilterFromJson(bodyJson);
        const QString customTitle = bodyJson.value("title").toString();
        const QString appendixHtml = bodyJson.value("appendix").toString();

        QList<Nullock::Core::Finding> findings;
        int totalBeforeFilter = 0;
        if (m_wiring.scanner) {
            const auto allFindings = m_wiring.scanner->findings(0);
            totalBeforeFilter = allFindings.size();
            for (const auto &f : allFindings)
                if (ControlLogic::findingPasses(f.severity, f.confidence, f.kind, /*fixed=*/false,
                                                rfc.minSeverityRank, rfc.minConfidenceRank,
                                                rfc.includeKinds, rfc.excludeKinds, rfc.includeFixed))
        ...
        const bool reportFiltered = findings.size() != totalBeforeFilter;
        const PostureGrade pg = computePostureGrade(findings);  // over the INCLUDED set
        ...
        h += "<title>" + esc(customTitle.isEmpty() ? (QStringLiteral("Nullock report — ") + proj) : customTitle) + "</title><style>"
        ...
        h += "<h1>" + esc(customTitle.isEmpty() ? QStringLiteral("Nullock engagement report") : customTitle) + "</h1>";
        h += "<p class=\"sub\">Project <strong>" + esc(proj) + "</strong> &middot; generated "
             + esc(QDateTime::currentDateTime().toString(Qt::ISODate));
        if (reportFiltered)
            h += " &middot; showing " + QString::number(findings.size()) + " of "
                 + QString::number(totalBeforeFilter) + " findings (filtered)";
        h += "</p>";
        ...
        if (!appendixHtml.isEmpty()) {
            // Custom appendix: plain text, escaped, newlines -> <br> (no raw HTML
            // so operator/report text can never inject markup into the document).
            h += "<h2>Appendix</h2><p>" + esc(appendixHtml).replace(QStringLiteral("\n"), QStringLiteral("<br>")) + "</p>";
        }
