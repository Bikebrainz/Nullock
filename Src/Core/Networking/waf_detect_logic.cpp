// Pure WAF/CDN/LB detection logic, split out of waf_detect.cpp so a unit test
// can link it (via Networking) against Qt6::Core alone -- detect() and its
// HttpClient (the Qt6::Network chain) stay in waf_detect.cpp. Mirrors the
// established sibling pattern (http_fingerprint_logic.cpp, takeover_logic.cpp,
// ...). Everything here is I/O-free: it matches a fixed signature table against
// already-fetched response headers, and renders a self-protecting GET.
...
// OR a cookie match (headerName "" + cookieNeedle => some Set-Cookie NAME
...
// keeping the first (strongest-evidence-first) match.
struct Sig {
    const char *name;
    const char *kind;
...
const QList<Sig> &sigs() {
    static const QList<Sig> t = {
        ...
        { "Signal Sciences",    "waf",   "x-sigsci-requestid", "", "" },
...
QByteArray buildGet(const Request &req) {
    ...
    for (const auto &h : req.headers) {
        ...
        // forces (Accept-Encoding: identity line 96, Connection: close line 103):
        //  - Accept-Encoding: else the server may compress, negating the forced identity.
        //  - Connection: a carried "keep-alive" contradicts the forced close (risking a
        //    hung/mis-framed read).
